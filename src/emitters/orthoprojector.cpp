...
.. _emitter-orthoprojector:

Orthographic Projection light source (:monosp:`orthoprojector`)
---------------------------------------------
 ...
 * - irradiance
   ...
   - 2D texture specifying irradiance on the emitter's virtual image plane,
     ...
     sensitivity profile (a.k.a. importance) of a orthographic camera. This
...
This emitter is the reciprocal counterpart of the orthographic camera
implemented by the :ref:`orthographic <sensor-orthographic>` plugin. It
accepts exactly the same parameters and employs the same pixel-to-direction
mapping. In contrast to the orthographic camera, it takes an extra texture
...
MI_VARIANT class OrthoProjector final : public Emitter<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Emitter, m_flags, m_to_world, m_needs_sample_3)
    MI_IMPORT_TYPES(Texture)

    OrthoProjector(const Properties &props) : Base(props) {
    ...
    void parameters_changed(const std::vector<std::string> &keys = {}) override {
        if (keys.empty() || string::contains(keys, "irradiance")) {
            ...
            m_camera_to_sample = orthographic_projection(size, size, 0,
                                                        ScalarFloat(1e-4f),
                                                        ScalarFloat(1e4f));
    ...
    sample_direction(const Interaction3f &it, const Point2f & /*sample*/,
         ...
         * See the weight returned by \ref orthographicCamera::sample_direction
         * and the comments in \ref orthographicCamera::importance.
         * Note that:
...
MI_IMPLEMENT_CLASS_VARIANT(OrthoProjector, Emitter)
MI_EXPORT_PLUGIN(OrthoProjector, "Orthographic projection emitter")
